/* Checks a *live* site, not the working tree.
 *
 *   node test/monitor.mjs https://you.github.io/your-conference/
 *   node test/monitor.mjs http://localhost:8765/
 *
 * The URL is required rather than defaulted: this repo has no deployment of its
 * own, and a monitor that silently checks somebody else's site is worse than no
 * monitor. Point it at yours once you have one, and wire it into
 * .github/workflows/check.yml on a schedule — a failing scheduled workflow is
 * the notification.
 *
...
if (!TARGET) {
  console.error("usage: node test/monitor.mjs <url-of-your-deployed-site> [--quiet]");
  process.exit(2);
}
 ...
 *
 * REPLACE THIS BLOCK. It asks Ex Ordo how many sessions RGS-IBG 2026 has and
 * compares that to the number shipped, which is the check that catches a
 * programme changing under a deployed copy — a moved session is invisible,
 * because the route still renders, it just sends people to the wrong room at
 * the wrong time. The idea ports; the endpoint and the dates do not. Until you
 * repoint it at your own source this section will simply fail. */
