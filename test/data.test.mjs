...
/* This repo ships without a conference, so on a fresh clone there is nothing
 * here to check — and that is not a failure. Skipping *loudly*, though: "0
 * checks, all passed" is precisely how a port ends up never running this file,
 * and every check in it guards a failure with no visible symptom. */
if (!existsSync(join(root, "docs/data/meta.json"))) {
  console.log("\nSKIPPED — no programme data in docs/data/.\n");
  console.log("  Run the three pipeline steps and then run this again:");
  console.log("    python3 pipeline/fetch.py && python3 pipeline/normalize.py && python3 pipeline/embed.py");
  console.log("  It is the gate on a pipeline run. See docs/data/README.md.\n");
  process.exit(0);
}
